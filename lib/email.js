...
export async function sendBookingConfirmation(enquiry) {
  if (!process.env.RESEND_API_KEY) {
    console.error('[Email] RESEND_API_KEY not set — skipping customer confirmation');
    return;
  }
  if (!process.env.FROM_EMAIL) {
    console.error('[Email] FROM_EMAIL not set — skipping customer confirmation');
    return;
  }
  console.log(`[Email] Sending confirmation to ${enquiry.email} from ${process.env.FROM_EMAIL}`);
  const preferredDate = enquiry.preferred_date
    ? new Date(enquiry.preferred_date).toLocaleDateString('en-GB', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })
    : 'To be confirmed';

  await getResend().emails.send({
    from: process.env.FROM_EMAIL,
    to: enquiry.email,
    subject: `Tree Monkey Tree Care - Enquiry confirmed #${enquiry.id}`,
    html: `
<!DOCTYPE html>
<html>
<body style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:20px;color:#333">
  ...
  <div style="border:1px solid #e0e0e0;border-top:none;padding:24px;border-radius:0 0 8px 8px">
    ...
    <table style="width:100%;border-collapse:collapse;margin:16px 0">
      <tr style="background:#f5f5f5"><td style="padding:8px 12px;font-weight:bold">Reference</td><td style="padding:8px 12px">#${enquiry.id}</td></tr>
      ...
      <tr><td style="padding:8px 12px;font-weight:bold">Postcode</td><td style="padding:8px 12px">${enquiry.postcode}</td></tr>
      ${enquiry.tpo_risk ? `<tr style="background:#fff3cd"><td style="padding:8px 12px;font-weight:bold;color:#856404">TPO note</td><td style="padding:8px 12px;color:#856404">A Tree Preservation Order may apply - we will advise on this during your site visit.</td></tr>` : ''}
    </table>
...
export async function sendOpsAlert({ subject, body }) {
  if (!process.env.RESEND_API_KEY) {
    console.error('[Email] RESEND_API_KEY not set — skipping ops alert');
    return;
  }
  if (!process.env.OPS_EMAIL) {
    console.log(`[Email] Ops alert (no OPS_EMAIL configured): ${subject}`);
    return;
  }
  console.log(`[Email] Sending ops alert: ${subject} → ${process.env.OPS_EMAIL}`);
